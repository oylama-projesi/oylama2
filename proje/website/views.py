...
from website.forms import GroupForm
from .models import Group, Member, OylamaForm,Poll, User,Vote,PollOptions
from . import db
import json
...
from .forms import OylamaForm, GroupForm
...
def create_poll():
    ...
    if form.validate_on_submit():
        ...
        # Oy verme kodunu oluştur
        vote_code = create_vote_code()

        new_poll = Poll(question=question, group_id=group_id, created_by=current_user.id, vote_code=vote_code, end_time=end_time)

        for option in options:
            new_poll_option = PollOptions(option=option, poll=new_poll)
            db.session.add(new_poll_option) 
...
def vote(poll_id):
    ...
    if poll.end_time and datetime.now() > poll.end_time:
        flash('Bu oylamanın süresi doldu!', category='error')
...
def view_results(poll_id):
    poll = Poll.query.get(poll_id)
    if poll.end_time and datetime.now() < poll.end_time:
        flash('Bu oylama henüz sona ermedi!', category='error')
        return redirect(url_for('views.list_polls'))
